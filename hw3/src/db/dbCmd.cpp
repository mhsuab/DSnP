...
DBAppendCmd::exec(const string& option)
{
    ...
    int value;
    bool check_if_option = false;
    check_if_option = CmdExec::lexOptions(option, options);
    if (!check_if_option)
            return CMD_EXEC_ERROR;
...
{  
    ...
    if (!CmdExec::lexSingleOption(option, token, false))
        return CMD_EXEC_ERROR;
    */
    //check_if_option = CmdExec::lexSingleOption(option, token, false);
    //if (!check_if_option)
    //if (option == "")
    /*{
        cout << dbjson << endl;
    ...
    else
    {
        ...
        for (size_t i = 0; i < dbjson.size(); i++)
        {
            ...
            //if (option == dbjson._obj[i].key())
            {
                check_option = true;
                cout << "{ " << dbjson[i] << " }" << endl;
                //cout << "{ " << dbjson._obj[i] << " }" << endl;
            }
        }
        if (!check_option) cerr << "Error: No JSON element with key \"" << option << "\" is found." << endl;
    }
    return CMD_EXEC_DONE;
}
